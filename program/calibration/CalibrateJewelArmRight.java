 ...
 */
@TeleOp(group = "calibration", name = "Calibrate Jewel Arm Right")
public class CalibrateJewelArmRight extends LinearOpMode {
    @Override
    public void runOpMode() throws InterruptedException {
        ...
        while(opModeIsActive()) {
            Servo jewelArm = hardwareMap.servo.get(Nerva.JEWEL_ARM_RIGHT_KEY);
            double upSetpoint = settings.getDouble("jewel_arm_right_up");
            ...
            double downSetpoint = settings.getDouble("jewel_arm_right_down");
            while(opModeIsActive() && !gamepad1.a) {
                if(Math.abs(gamepad1.left_stick_x) > 0.05) {
            ...
            double centerSetpoint = settings.getDouble("jewel_arm_right_center");
            while(opModeIsActive() && !gamepad1.a) {
                if(Math.abs(gamepad1.left_stick_x) > 0.05) {
            ...
            settings.setDouble("jewel_arm_right_down", downSetpoint);
            settings.setDouble("jewel_arm_right_up", upSetpoint);
            settings.setDouble("jewel_arm_right_center", centerSetpoint);
            settings.save();
